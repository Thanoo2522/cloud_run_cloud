...
# ================= API: GET CONFIG =================
@app.route("/config/<ofm>", methods=["GET", "OPTIONS"])
def get_config_api(ofm):
    # ป้องกันปัญหาเรื่องภาษาไทยและช่องว่าง
    import urllib.parse
    decoded_ofm = urllib.parse.unquote(ofm).strip()
    ...
    if not config:
        return jsonify({
            "status": "error",
            "message": f"ไม่พบ config ของ {decoded_ofm}"
        }), 404

    # ตรวจสอบว่าได้ค่า liffId และ apiUrl จริงๆ หรือไม่
    liff_id = config.get("liffId")
    api_url = config.get("apiUrl")

    if not liff_id or not api_url:
         return jsonify({
            "status": "error",
            "message": "ข้อมูลใน Firebase ไม่ครบ (เช็ค liffId/apiUrl)"
        }), 500
...
def webhook():
    try:
        ...
        print("📩 BODY:", json.dumps(body_json, ensure_ascii=False))
        ...
        for event in events:
            ...
            if not is_registered:
                ...
                text_msg = (
                    ...
                    if liff_id else "❌ ระบบยังไม่พร้อม"
                )
